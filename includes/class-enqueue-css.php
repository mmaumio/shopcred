...
class ShopCred_Enqueue_CSS {
	...
	public function enqueue_assets() {
		$dynamic_css_priority = apply_filters( 'ShopCred_Dynamic_CSS_priority', 10 );
		...
		add_action( 'wp_footer', array( $this, 'print_inline_js' ), 20 );
	}
	...
	public function page_id() {
		...
		$id = ShopCred_Helper::is_shopcred() ? get_option( 'spc_wc_shop_id' )['spc_wc_shop_field'] : $id;
	...
	public function mode() {
		...
		$mode = apply_filters( 'shopcred_css_print_method', 'file' );
		...
		if ( 'file' === $mode && $this->needs_update() ) {
			...
			$last_time    = (int) get_option( 'shopcred_dynamic_css_time' );
	...
	public function enqueue_dynamic_css() {
		...
		$has_shopcred = get_post_meta( $page_id, '_shopcred_dynamic_css_version', true );

		if ( empty( $has_shopcred ) ) {
			return;
		}

		if ( 'file' === $this->mode() ) {
			wp_enqueue_style( 'shop-cred', esc_url( $this->file( 'uri' ) ), array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
	}
	...
	public function print_inline_css() {
		if ( 'inline' === $this->mode() || ! wp_style_is( 'shop-cred', 'enqueued' ) ) {
			$css = ShopCred_GenerateCSS::get_frontend_block_css();
	...
	public function print_inline_js() {
		if ( 'inline' === $this->mode() ) {
			...
			$js = ShopCred_GenerateCSS::get_dynamic_js($content);
			...
			printf(
				'<script id="shopcred-js">%s</script>',
				wp_strip_all_tags( $js ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			);
		}
	}
	...
	public function make_css() {
		...
		$content = ShopCred_GenerateCSS::get_frontend_block_css();
		...
		if ( $css_size < (int) apply_filters( 'shopcred_css_inline_length', 500 ) ) {
			return false;
		}
		...
		if ( is_writable( $this->file( 'path' ) ) || ( ! file_exists( $this->file( 'path' ) ) && is_writable( dirname( $this->file( 'path' ) ) ) ) ) {
			...
			if ( ! $wp_filesystem->put_contents( $this->file( 'path' ), wp_strip_all_tags( $content ), $chmod_file ) ) {
				...
				$option             = get_option( 'ShopCred_Dynamic_CSS_posts', array() );
				$option[ $page_id ] = true;
				update_option( 'ShopCred_Dynamic_CSS_posts', $option );

				// Update the 'shopcred_dynamic_css_time' option.
	...
	public function can_write() {
		...
		$folder_path = $upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'shop-cred';
	...
	public function file( $target = 'path' ) {
		...
		$css_uri = trailingslashit( $css_uri_folder ) . 'shop-cred/' . $file_name;
	...
	public function add_options() {
		/**
		 * The 'ShopCred_Dynamic_CSS_posts' option will hold an array of posts that have had their css generated.
		 * We can use that to keep track of which pages need their CSS to be recreated and which don't.
		 */
		add_option( 'ShopCred_Dynamic_CSS_posts', array(), '', 'yes' );

		/**
		 * The 'shopcred_dynamic_css_time' option holds the time the file writer was last used.
		 */
		add_option( 'shopcred_dynamic_css_time', time(), '', 'yes' );
	}

	/**
	 * Update the ShopCred_Dynamic_CSS_posts option when a post is saved.
	 *
	...
	public function post_update_option( $post_id, $post ) {
		...
		if ( isset( $post->post_content ) ) {
			if ( strpos( $post->post_content, 'wp:spc' ) !== false ) {
				update_post_meta( $post_id, '_shopcred_dynamic_css_version', sanitize_text_field( SPC_VER ) );
			} else {
				delete_post_meta( $post_id, '_shopcred_dynamic_css_version' );
			}
			...
			if ( ! empty( $stored_reusable_blocks ) ) {
				...
				update_post_meta( $post_id, '_shopcred_reusable_blocks', $stored_reusable_blocks );
			} else {
				delete_post_meta( $post_id, '_shopcred_reusable_blocks' );
			}
		}
	...
	public function wp_block_update( $post_id, $post ) {
		...
		if ( isset( $post->post_content ) ) {
			if ( strpos( $post->post_content, 'wp:spc' ) !== false ) {
				...
				$posts = $wpdb->get_col( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_shopcred_reusable_blocks'" );
	...
	public function update_saved_time() {
		update_option( 'shopcredblocks_dynamic_css_time', time() );
	}
...
ShopCred_Enqueue_CSS::get_instance();
